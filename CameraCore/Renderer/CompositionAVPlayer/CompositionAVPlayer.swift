...
extension Renderer {
	public class CompositionAVPlayer: NSObject, CompositionAVPlayerProtocol, RenderEventControllerProtocol {
		public var events: [RenderEventProtocol] = []
		
		public typealias Status = CompositionAVPlayerStatus
		public typealias ErrorType = CompositionAVPlayerErrorType
		
		public fileprivate(set) var status: CompositionAVPlayer.Status = .setup {
			willSet {
				self.event?.onStatusChange?(newValue)
			}
		}
		public fileprivate(set) var compositionData: CompositionDataProtocol?
		...
		public var event: CompositionAVPlayerEvent?
		...
		public func setup(compositionData: CompositionDataProtocol) throws {
			...
			if compositionData.videoComposition != nil {
				self.output = AVPlayerItemVideoOutput(pixelBufferAttributes: [
					...
					kCVPixelBufferWidthKey as String : compositionData.property.renderSize.width,
					kCVPixelBufferHeightKey as String : compositionData.property.renderSize.height,
					kCVPixelFormatOpenGLESCompatibility as String : true,
					])
				self.output?.setDelegate(self, queue: self.queue)
...
extension Renderer.CompositionAVPlayer {
...
extension Renderer.CompositionAVPlayer: AVPlayerItemOutputPullDelegate {
