...
extension Renderer.VideoCapture {
	final public class CaptureWriter {
		...
		public struct Paramator {
			...
			public init (outputFilePath: URL, presetiFrame: Settings.PresetiFrame, frameRate: Int32, devicePosition: AVCaptureDevice.Position, croppingRect: CGRect?, fileType: AVFileType = AVFileType.mp4, videoCodecType: Settings.VideoCodec = .h264) {
		...
		static func addCaptureSampleBuffer(sampleBuffer: CMSampleBuffer) {
			...
			if w.status == AVAssetWriter.Status.failed {
				Debug.ActionLog("writer status faild")
				return
			}
		...
		static func _addVideoBuffer(image: CIImage, sampleBuffer: CMSampleBuffer, timestamp: CMTime) {
			...
			if let buf: CVPixelBuffer = outputRenderBuffer {
				...
				SharedContext.ciContext.render(resultImage, to: buf, bounds: resultImage.extent, colorSpace: Configuration.colorSpace)
